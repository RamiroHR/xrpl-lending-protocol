...
**Yield mechanics.** Investor yield is delivered entirely through Price Per Share (PPS) appreciation — there is no separate interest disbursement or hardcoded rate. Shares are minted at subscription-time PPS (initially 1.0 XRP per share). Two events during Investment raise PPS by adding XRP to `AssetsTotal` without minting new shares: the broker's coupon injection via `tfVaultDonation` and the borrower's repayment of principal plus interest. At Redemption, investors burn shares at the current (higher) PPS and receive back more XRP than they deposited; the difference is their yield. V1.1's cash-basis model means PPS does not move at loan origination — only when cash lands in the vault.
